...
module.exports = async function (interaction) {
    ...
    let prevcheck = await prisma.absenceRole.findFirst({where: {id: roleID.toString()}});
    if(prevcheck) {
        prisma.absenceRole.delete({where: {id: roleID.toString()}}).then(() => {
            return interaction.reply({
                ...
                embeds: [
                    {
                        title:
                            "✅ | Removed the role from the database.",
                        color: null,
                        footer: {
                            text: "BTE Teamtool",
                            icon_url: "https://bte-germany.de/logo.gif",
    ...
    prisma.absenceRole.create({
        ...
        return interaction.reply({
            ...
            embeds: [
                {
                    title:
                        "✅ | Added the role to the database",
                    color: null,
                    footer: {
                        text: "BTE Teamtool",
                        icon_url: "https://bte-germany.de/logo.gif",
